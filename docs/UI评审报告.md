...
| 个人主页 | profile.png | 8.5 | 头像（可上传更换）/关注·粉丝（可点列表）/累计完成/TA 的帖子/设置入口 |
| **我的计划独立页** | plans.png | **9** | 晨间计划（自建）/提醒数/椭圆开关（滑块尺寸匹配）/添加提醒/一键发帖/删除 |

## 2.1 第三/四轮迭代复评（2026-08-29 晚）
- **社交页**：头像圈（白底绿字，无月牙异形）；tabs **黏性吸顶**（滚动不再穿插遮挡）；帖子整卡可点进详情；图片 `aspect-video object-cover` 防底部裁切；时间「16 分钟前 · 已编辑」（修复 future 时间显示"分钟前"）；发布弹窗支持**引用计划 + 图片上传**——评分 **9**
- **帖子详情**：媒体图容器 overflow-hidden；「发布于 xx · 已编辑」；编辑（内联）/删除（ConfirmModal 确认）；加入/退出**乐观更新**——评分 **9**
- **提醒列表**：⚙ 显示设置弹窗（分类显隐，localStorage 持久化）+ 筛选空态；计划提醒「来自计划 + 已修改」徽标——评分 **8.5**
- **看板**：喝水卡「✓ 目标达成」徽章 + 「+200ml ✦」浮字动画——评分 **8.5**
